...
class PutawayService {
  ...
  Future<List<Map<String, dynamic>>> locations(String warehouseId) async {
    final r = await http.get(
      Uri.parse(
        '$base/putaway/locations?warehouseId=${Uri.encodeComponent(warehouseId)}',
      ),
      headers: headers,
    );
    if (r.statusCode != 200) throw Exception(_err(r));
    return ((jsonDecode(r.body) as Map)['locations'] as List? ?? [])
  ...
  Future<List<Map<String, dynamic>>> pending() async {
    final r = await http.get(
      Uri.parse('$base/putaway/pending'),
      headers: headers,
    );
    if (r.statusCode != 200) throw Exception(_err(r));
    return ((jsonDecode(r.body) as Map)['items'] as List? ?? [])
        .map((e) => Map<String, dynamic>.from(e as Map))
        .toList();
  }
  ...
  Future<List<Map<String, dynamic>>> tasks() async {
    final r = await http.get(
      Uri.parse('$base/putaway'),
      headers: headers,
    );
    if (r.statusCode != 200) throw Exception(_err(r));
    return ((jsonDecode(r.body) as Map)['tasks'] as List? ?? [])
        .map((e) => Map<String, dynamic>.from(e as Map))
        .toList();
  }
  ...
  Future<Map<String, dynamic>> create({
    ...
    return Map<String, dynamic>.from(
      (jsonDecode(r.body) as Map)['task'] as Map,
    );
  }

  Future<void> process(String id, String locationId) async {
    final r = await http.post(
      Uri.parse('$base/putaway/$id/process'),
      headers: headers,
  ...
  String _err(http.Response r) {
    try {
      ...
      return b['error']?['message']?.toString() ?? 'Request failed.';
    } catch (_) {
