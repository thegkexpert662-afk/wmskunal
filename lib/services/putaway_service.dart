...
class PutawayService {
  ...
  Future<Map<String,dynamic>> create({required String sourceType,required String sourceItemId,required String locationId,required double quantity})async{
  ...
  Future<void> process(String id,String locationId)async{
    final r=await http.post(Uri.parse('$base/putaway/$id/process'),headers:headers,body:jsonEncode({'locationId':locationId,'quantity':0.000001}));
    if(r.statusCode!=200)throw Exception(_err(r));
  }
