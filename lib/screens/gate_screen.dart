...
class _GateScreenState extends State<GateScreen> {
  ...
  Widget _panel(String title, Widget body, {String? action}) => Container(
        padding: const EdgeInsets.all(15),
        decoration: _box(),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [Text(title, style: _bold(14)), const Spacer(), if (action != null) Text(action, style: const TextStyle(color: Color(0xFF1769E8), fontSize: 11, fontWeight: FontWeight.w700))]),
          const SizedBox(height: 8),
          body,
        ]),
      );

  Widget _table() => _panel('Gate Movement Records', Column(children: [
        _filters(),
        ...
        SingleChildScrollView(
          ...
          child: DataTable(
            ...
            columns: const [
              DataColumn(label: Text('#')), DataColumn(label: Text('Gate ID')), DataColumn(label: Text('Vehicle No')), DataColumn(label: Text('Client')), DataColumn(label: Text('Warehouse')), DataColumn(label: Text('Type')), DataColumn(label: Text('Time')), DataColumn(label: Text('Status')), DataColumn(label: Text('Actions')),
            ],
            rows: List.generate(filtered.length, (i) {
              final r = filtered[i];
              return DataRow(cells: [
                DataCell(Text('${i + 1}')), DataCell(Text(r[0])), DataCell(Text(r[1], style: const TextStyle(fontWeight: FontWeight.w700))), DataCell(Text(r[2])), DataCell(Text(r[3])), DataCell(_typeChip(r[4])), DataCell(Text(r[5])), DataCell(_statusChip(r[6])), DataCell(_view(r[0])),
              ]);
            }),
          ),
        ),
        const Divider(height: 1),
        Padding(padding: const EdgeInsets.symmetric(vertical: 10), child: Row(children: [Text('Showing 1 to ${filtered.length} of 1,248 entries', style: const TextStyle(color: Color(0xFF718096), fontSize: 10)), const Spacer(), _page('Prev'), _page('1', true), _page('2'), _page('3'), _page('4'), _page('5'), _page('…'), _page('125'), _page('Next')]))
      ]));

  Widget _filters() => Padding(
        padding: const EdgeInsets.all(10),
        child: Wrap(spacing: 8, runSpacing: 8, children: [
          SizedBox(width: 310, height: 40, child: TextField(controller: search, decoration: InputDecoration(hintText: 'Search vehicle, gate ID, client...', hintStyle: const TextStyle(fontSize: 10, color: Color(0xFF8290A2)), prefixIcon: const Icon(Icons.search_rounded, size: 18), contentPadding: const EdgeInsets.symmetric(vertical: 9), border: OutlineInputBorder(borderRadius: BorderRadius.circular(8), borderSide: const BorderSide(color: Color(0xFFDCE4EE))))),
          _drop(type, const ['All', 'Inward', 'Outward'], (v) => setState(() => type = v!), 'Type'),
          _drop(status, const ['All', 'Inside', 'Exited', 'Pending'], (v) => setState(() => status = v!), 'Status'),
          ...
          OutlinedButton.icon(onPressed: () => _msg('Gate records exported successfully.'), icon: const Icon(Icons.download_outlined, size: 16), label: const Text('Export'), style: OutlinedButton.styleFrom(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 11))),
        ]),
      );
  ...
  Widget _typeChip(String s) {
    final inward = s == 'Inward';
    final color = inward ? const Color(0xFF1769E8) : const Color(0xFF7447D8);
    final bg = inward ? const Color(0xFFEAF2FF) : const Color(0xFFF0EAFF);
  ...
  Widget _view(String id) => IconButton(onPressed: () => _msg('Viewing gate record $id'), icon: const Icon(Icons.visibility_outlined, color: Color(0xFF1769E8), size: 17), tooltip: 'View');
