...
class _ClientPortalScreenState extends State<ClientPortalScreen> {
  ...
  Widget _panel(String title, Widget child) {
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(title, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w800, color: Color(0xFF162B46))),
            ...
            child: Row(
              children: [
                Container(
                  ...
                  child: const Icon(Icons.business_outlined, color: Color(0xFF1769E8), size: 25),
                ),
                ...
                const Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text('ABC Industries', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w800, color: Color(0xFF172A43))),
                      SizedBox(height: 3),
                      Text('CL-001 • Brand A', style: TextStyle(fontSize: 9, color: Color(0xFF718096))),
                    ],
                  ),
                ),
                _statusChip('Active'),
  ...
  Widget _orderRow(String id, String sub, String statusText, Color color) {
    return Container(
      margin: const EdgeInsets.only(bottom: 7),
      padding: const EdgeInsets.symmetric(horizontal: 9, vertical: 8),
      decoration: BoxDecoration(color: const Color(0xFFF8FAFD), borderRadius: BorderRadius.circular(8)),
      child: Row(children: [Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(id, style: const TextStyle(fontSize: 10, fontWeight: FontWeight.w800, color: Color(0xFF26384E))), const SizedBox(height: 2), Text(sub, style: const TextStyle(fontSize: 8, color: Color(0xFF7A889A)))])), _smallStatus(statusText, color)]),
    );
  }
  ...
  Widget _ordersTable() {
    ...
    final filtered = orderStatus == 'All' ? rows : rows.where((r) => r[5] == orderStatus).toList();
    return _panel(
      'My Orders',
      Column(
        children: [
          Align(
            ...
            child: _dropdown(orderStatus, const ['All', 'Processing', 'Dispatched', 'Delivered'], (value) => setState(() => orderStatus = value ?? 'All')),
          ),
          ...
          SingleChildScrollView(
            ...
            child: DataTable(
              ...
              rows: filtered.map((r) => DataRow(cells: [
                ...
                DataCell(Text(r[3])),
                DataCell(Text(r[4])),
                DataCell(_smallStatus(r[5], _statusColor(r[5]))),
                DataCell(IconButton(onPressed: () => _message('Viewing ${r[0]}'), icon: const Icon(Icons.visibility_outlined, color: Color(0xFF1769E8), size: 17))),
              ])).toList(),
            ),
          ),
        ],
  ...
  Widget _invoicesTable() {
    ...
    return _panel(
      ...
      Column(
        children: [
          Align(alignment: Alignment.centerRight, child: OutlinedButton.icon(onPressed: () => _message('Invoice export started.'), icon: const Icon(Icons.download_outlined, size: 16), label: const Text('Export'))),
          const Divider(height: 1),
          SingleChildScrollView(
            scrollDirection: Axis.horizontal,
            child: DataTable(
              ...
              rows: rows.map((r) => DataRow(cells: [
                ...
                DataCell(Text(r[2])),
                DataCell(Text(r[3])),
                DataCell(_smallStatus(r[4], const Color(0xFF16A05D))),
                DataCell(_smallStatus(r[5], r[5] == 'Sent' ? const Color(0xFF16A05D) : const Color(0xFFE59A00))),
                DataCell(IconButton(onPressed: () => _message('Opening ${r[0]}'), icon: const Icon(Icons.visibility_outlined, color: Color(0xFF1769E8), size: 17))),
              ])).toList(),
            ),
          ),
        ],
  ...
  Widget _info(String title, String value, IconData icon) {
    return Padding(
      ...
      child: Row(children: [Icon(icon, size: 15, color: const Color(0xFF748397)), const SizedBox(width: 8), Expanded(child: Text(title, style: const TextStyle(fontSize: 9, color: Color(0xFF748397)))), Text(value, style: const TextStyle(fontSize: 9, fontWeight: FontWeight.w700, color: Color(0xFF293C53)))]),
    );
  }
  ...
  Widget _statusChip(String text) {
    return Container(
      ...
      decoration: BoxDecoration(color: const Color(0xFFE4F7EC), borderRadius: BorderRadius.circular(6)),
      child: const Text('Active', style: TextStyle(color: Color(0xFF14894E), fontSize: 8, fontWeight: FontWeight.w800)),
    );
  }
